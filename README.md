...
- [주요 구현 기능](#주요-구현-기능)
  * [1. ALB 기반 서버 인스턴스 관리](#3-ALB-기반-서버-인스턴스-관리)
  * [2. 프라이빗 서브넷 내 스토리지 인프라 구성](#4-프라이빗-서브넷-내-스토리지-인프라-구성)
  * [3. 추천 검색어 조회](#1-추천-검색어-조회)
  * [4. 인기 검색어 조회](#2-인기-검색어-조회)
...
- [검색 품질 및 성능 개선](#검색-품질-및-성능-개선)
  ...
  - [성능 개선](#성능-개선)
    * [1. primary, replica 샤드 개수 조정](#2-primary,-replica-샤드-개수-조정)
    * [2. 검색 로그 Index - force merge](#3-검색-로그-Index---force-merge)
...
> 목표 : 데이터 파이프라인과 웹 서비스간의 부하 분산
<img width="583" alt="image" src="https://github.com/user-attachments/assets/331a76f2-b134-400c-9036-b75910d90596">


- V사의 데이터 포털 인스턴스와 Airflow 인스턴스를 모두 회사 내부망에서 관리할 수 있도록 `private subnet`에 배치시켰다.
- 구식 CLB과는 다르게 ALB는 로드 밸런서 하나만으로 각 대상그룹에 라우팅을 시켜줬다.
- `path`뿐만 아니라 `port` 에 따라 대상그룹을 매핑시켜 줄 수 있는 장점이 있다.
- `8000`과 `8080` 포트를 활용해 데이터 파이프라인과 웹 서비스 그룹으로의 요청을 분산시켜 서버의 과부하를 방지하는 결과를 내었다. 


### 2. 프라이빗 서브넷 내 스토리지 인프라 구성  

> 목표 : 외부 접속이 불가한 private subnet 내의 스토리지 자원 관리

- `NAT Gateway` : 외부 접속이 불가한 AWS RDS 내 업데이트를 위해선, 내부에서 외부로 접속할 수 있는 수단이 필요하다. 따라서, 사설 IP를 공인 IP로 바꿔주는 역할의 NAT Gateway를 사용하여 내부에서 외부 접속이 가능한 환경을 구축했다.

- `Bastion Host` : SSH로 외부에서 인스턴스에 접속해 직접 자원을 조정할 일이 생길걸 대비하여, public subnet에 Bastion Host를 생성해줬다. `SSH jump host`에 Bastion 인스턴스의 IP주소를 입력하여 접속이 가능하게끔 해줬다.
...
시험, TEST
```
![image](https://github.com/shinsj4653/vs-data-portal-backend/assets/49470452/92a49221-104e-4f0a-bf78-53abf6b66499)  
*V사 표준용어사전의 표준단어정의서 섹션*
...
시험과 TEST도 표준용어사전에 등록되어 있기에, 동의어로 처리해주면 의미 있는 검색 결과를 낼 수 있겠다고 판단하였다.
* * *
`구현 2. 사용자 사전`  
...
반면, 개수가 너무 적다면 병렬 처리 효과를 크게 낼 수 없어 분산도가 떨어진다.  
